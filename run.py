...
MEDIUM_WORDS = [
    "grape", "lemon", "tiger", "chair", "piano",
    "zebra", "wheat", "train", "plane", "beach",
    "camel", "flute", "globe", "horse", "knife",
    "monkey", "peach", "robot", "shark", "torch"
]

HARD_WORDS = [
    "elephant", "computer", "astronaut", "microwave",
    "television", "saxophone", "kaleidoscope", "encyclopedia",
    "hippopotamus", "chrysanthemum", "binoculars", "circumference",
    "constellation", "laboratory", "thermometer", "university",
    "vocabulary", "xylophone", "yesterday", "zoologist"
]
...
class HangmanGame:
    ...
    def choose_game_level(self):
        """
        Prompt the player to choose the difficulty level of the
        game and return the corresponding word list.

        Returns:
            list: A list of words corresponding to the chosen difficulty level.

        while : validates that the player is typing the level correctly
        ...
        while True:
            ...
            if self.level == "easy":
                ...
                print(f'Invalid level "{Fore.RED}{self.level}{Fore.RESET}".')
...
def start_game(game, word_list):
    ...
    Args:
        ...
        word_list (list): The list of words to choose from based
...
def update_display(game):
    """
    Update and print the display to show the current state of the game,
    including the word progress and number of misses.
    ...
    display_word = " ".join([
        letter if letter in game.guesses else "_" for letter in game.word
        ])
...
def check_guess(game, guess):
    """
    Check the player's guessed letter, update the game state,
    and handle invalid inputs with error messages.
